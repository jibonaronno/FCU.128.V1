...
//REM		outb(LCD_DATA_POUT, inb(LCD_DATA_POUT)|0xFF);	// set pull-ups to on (4bit)
		delay_ms(1);
	#else
...
//		delay_ms(2);
		...
		outb(LCD_DATA_DDR, ((inb(LCD_DATA_DDR)&0b00000000)|0b11111111));	// set data I/O lines to output (4bit)

		outb(LCD_DATA_POUT, (inb(LCD_DATA_POUT)&0xF0) | ((data>>4)&0x0F) );
...
//		delay_ms(2);
		...
		outb(LCD_DATA_POUT, (inb(LCD_DATA_POUT)&0xF0) | ((data)&0x0F) );
...
// write a data byte to the display
	...
	cbi(LCD_CTRL_PORT, LCD_CTRL_RW);		// set R/W to "write"
		...
		LCD_DELAY;								// wait
//		LCD_DELAY;								// wait
		outb(LCD_DATA_DDR, inb(LCD_DATA_DDR)|0b11111111);	// set data I/O lines to output (4bit)
		outb(LCD_DATA_POUT, (inb(LCD_DATA_POUT)&0xF0) | ((data>>4)&0x0F) );	// output data, high 4 bits
